...
#===============================================================================
# function for re-estimating the betas / tree predictions

refit_betas <- function( Y = NULL, Z = NULL, PHI = NULL, ID = NULL, D = NULL, 
                         Sigma2 = NULL)
{
  ...
  for (ii in 1:NID) {
    ...
    InvVi <- solve( Vi )
    #- compute parts
    tmpBeta1 <-  tmpBeta1 + t(PHIi) %*% InvVi %*% PHIi
    tmpBeta2 <-  tmpBeta2 + t(PHIi) %*% InvVi %*% Yi
  }
  out <- solve( tmpBeta1 ) %*% tmpBeta2
  return( out )      
}
...
mem_boost_reem <- function( formula, data = NULL, random = NULL,
                            conv_memboost = 0.001, 
  ...
  while ( toIterate ) {
    ...
    #----------------------------------------------------------------
    #- (iii): re-estimate tree predictions and get an updated fhat
    #----------------------------------------------------------------
    leaves <- tmpGTB$leaves
    inbag <- tmpGTB$inbag
    resid <- tmpGTB$outcome
    fhat <- 0
    
    for ( m in 1:n.trees ) { 
      #- get leaves of tree:
      uniqueLeaves <- unique( leaves[, m ] )
      #- make Phi matrix
      PHI <- matrix( 0, TotalObs, length( uniqueLeaves ) )
      for ( cc in 1:dim(PHI)[2] ) {
        idx <- which( leaves[, m] == uniqueLeaves[cc])
        PHI[ idx, cc ] <- 1
      }
      #- get outcome that was used for training: 
      tmpY <- resid[ , m ]
      
      if ( bag.fraction < 1 ) {
        inbag.idx <- which( inbag[, m ] == 1 ) #- get objects that were used for training:
        bnew <- refit_betas( Y = tmpY[inbag.idx], Z = Z[inbag.idx, , drop = FALSE], 
                             PHI = PHI[inbag.idx, , drop = FALSE], # drop = FALSE needed to ensure PHI stays a matrix even when there was no split
                             ID = ID[inbag.idx], D = Dhat, Sigma2 = Sigma2hat )
        
        #- replace the old bs with bnew in the tree object
        adjtarg <- cbind(uniqueLeaves, bnew)
        tmpGTB$trees[[m]]$frame[adjtarg[, 1], ]$yval <- adjtarg[, 2]
        
        #- update predictions
        fhat <- fhat + shrinkage * ( PHI%*%bnew ) 
        
      } else {
        bnew <- refit_betas( Y = tmpY, Z = Z, PHI = PHI, ID = ID, D = Dhat,
                             Sigma2 = Sigma2hat )
        
        #- replace the old bs with bnew in the tree object
        adjtarg <- unique(cbind(uniqueLeaves, bnew))
        tmpGTB$trees[[m]]$frame[adjtarg[, 1], ]$yval <- adjtarg[, 2]
        # print(tmpGTB$trees[[m]]$frame[adjtarg[, 1], ]$yval)
        # print(adjtarg[, 2])
    ...
    #- add initializing predictions: for L2, this is the mean of the outcome
    # the ensemble is fitted to,
    # for L1 and Huber, it's the median
    meanYstar <- mean( Ystar )
    medianYstar <- median( Ystar )
    if (loss == "L2") { 
      fhat <- fhat + meanYstar
    } else {
      fhat <- fhat + medianYstar 
    }
    ...
    if (modification == "beta") { # in case of Huber and L1 loss, use modified updating 
      ...
      #----------------------------------------------------
      #- (v): compute random effects and new epsilon
      ...
      #----------------------------------------------------
      #- (iv): compute new bs and new epsilons
      #----------------------------------------------------
      for ( ii in 1:NID ) {
        #- get index variable:
        idx <- which( ID == UniqueID[ii] )
      ...
      for ( ii in 1:NID ) {
        ...
        DhatNew <- DhatNew + ( bhat[ii,]%*%t(bhat[ii,]) + ( Dhat - Dhat%*%t(Zi)%*%InvVi%*%Zi%*%Dhat ) )
        tmpSigma <- as.numeric( Sigma2hat )*( ni - as.numeric( Sigma2hat )*sum( diag( InvVi ) ) ) 
        #tmpSigma <- as.numeric( Sigma2hat )*sum( diag( InvVi ) )
        Sigma2hatNew <- Sigma2hatNew + ( t(ehat[idx])%*%ehat[idx] + tmpSigma )     
      }
      #- update matrices:
      Dhat <- DhatNew/NID
