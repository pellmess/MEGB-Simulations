...
#==================================================================================================================================#
#-----------------------------------  Scenario 4: Interaction Pareto ---------------------------------------------------------------#
#==================================================================================================================================#

gen_x1 = function(dat, m = dat$muD, s = 2) {
  dat["x1"] = rnorm(nrow(dat), mean = m, sd = s)
  return(dat)
}

gen_x2 <- function(dat, m = dat$muD, s = 2) {
...
gen_myE = function(dat, shape = 3, scale = 800) {
  tmp =  sqrt(2) * actuar::rpareto(nrow(dat), shape = shape, scale = scale)
  dat["e"] = tmp - mean(tmp)
  return(dat)
}
...
setup = sim_base(data = base_id(nDomains = Domains, nUnits = pop_size)) %>%
  ...
  sim_gen_v(mean = 0, sd = 1000)         %>%
  sim_resp_eq(y =  20000 - 500 * x1 * x2 - 250 * (x2 ^ 2) + v + e)
...
sampler = function(DAT) {
  ...
  for (i in 1:Domains) {
    #YJ: to generate sample with 0 obs. for some domains
    if (sample_size[i] != 0) {
...
# Adding additional features to the sample
for (p in 1:simruns) {
  Pop[[p]]$sam <- NA
...
domain.pop.mean <-  t(sapply(
  Pop,
  FUN = function(X) {
    tapply(X$y, X$idD, mean)
  }
))
domain.pop.var <-  t(sapply(
  Pop,
  FUN = function(X) {
...
save.image("mod4/model_objects.RData")
save(input_elements,
     setup,
     aux.list,
     Domains,
     ...
     sample_size,
     time.sim,
     file = "mod4/model_specf_objects.RData")

alt.matrix <- data.frame(
  mean = as.vector(t(domain.pop.mean)),
  domain = rep(1:Domains, times = simruns),
  run = rep(1:simruns, each = Domains)
)
write.csv(alt.matrix, "mod4/results/pop.value.csv", row.names = F)
