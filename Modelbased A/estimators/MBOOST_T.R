for (i in r) {
  ...
  if(file.exists(paste0(path, i, ".txt"))){
    ...
    #-------------------------------------------------------------------------------
    #- Model-based boosting using trees as base-learners
    #-------------------------------------------------------------------------------
    ...
    fitMBOOST_T_L2 <- mboost::gamboost( y ~ btree(x1, x1) +
                                          brandom(idD),
    ...
    area_est_MBOOST_T_L2 <- cbind.data.frame(pop_data_pred, idd = input.elements.no$pop[, group.id]) |> group_by(idd) |> dplyr::summarise(mw = mean(pop_data_pred))

    names(area_est_MBOOST_T_L2) <- c("domain", "Mean")
    area_est_MBOOST_T_L2$run <- no
    
    
    csv_path <- paste0(path, "/", sprintf("%03d", no), "_mboost_l2_t_tree.csv")
    message(paste("csv_path:", csv_path))
    
    write.table(
      area_est_MBOOST_T_L2,
      file = csv_path,
      row.names = F,
      col.names = T
    )
    ...
    write(time_MBOOST_T_L2, file = time_path, ncolumns = 1)
